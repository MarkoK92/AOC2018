...
namespace AdventDay2
{
    class Challenge2
    {
        static void Main(string[] args)
        {
            ...
            //prvi del
            var dvojke = input.Count(id => id.GroupBy(c => c).Any(g => g.Count() == 2));
            var trojke = input.Count(id => id.GroupBy(c => c).Any(g => g.Count() == 3));

            var sum = dvojke * trojke;


            //drugi del

            //izločim ponovljene vrste
            input.Distinct();
            string skupneCrke = "";
            for (int i = 0; i < 25; i++)
            {
                //removam crke skozi string in prestejem če jih je več kot 1
                var par = input.Select(s => s.Remove(i, 1))
                    .GroupBy(d => d)
                    .FirstOrDefault(g => g.Count() > 1);

                if (par != null)
                {
                    skupneCrke = par.First();
                }

            }

            var končnString = skupneCrke;
